...
const UpdateMemberPage = ({params}) => {
  ...
  const [memberAlumni, setMemberAlumni] = useState(false);
  ...
  const handleUpdateMember = async () => {
    if (memberName.trim() && memberAbout.trim()) {
      ...
      formData.append("alumni", memberAlumni);
        ...
        router.push('/admin/peoples/current-member');
      } catch (error) {
    ...
    <div className="px-3">
      ...
      <input
        ...
        <input
          ...
          onChange={(e) => setMemberAlumni(e.target.checked)}
          className="form-checkbox"
        />
        <span className="ml-2 text-white font-bold ">Make Alumni</span>
      </label>
      <button
